...
const Signup = () => {
  ...
  const phase2 = () => {
    if (
      ...
      form.getValues().username.length <= 50
    ) {
  ...
  const phase3 = () => {
    if (
      ...
      form.getValues().password.length <= 50 &&
      form.getValues().password &&
      form.getValues().retype_password === form.getValues().password
    ) {
  ...
  function phoneGeneration() {
    ...
    handlePhone(`${phoneNumber}`, `${code}`);
  }

  function phoneVerification() {
    const phoneVerificationInput = document.getElementById(
      "verify_phone"
    ) as HTMLInputElement;
    const phoneVerification = phoneVerificationInput?.value
      ? phoneVerificationInput.value
      : null;

    console.log(phoneCode);

    if (phoneVerification === phoneCode) {
      console.log("approved", phoneCode, phoneVerification);
      return true;
    } else {
      console.log(phoneCode, phoneVerification);
      return false;
    }
  }

  const handleEmail = async (to: string, message: string) => {
  ...
  function emailGeneration() {
    ...
    setEmailCode(code);
    console.log(code, form.getValues("email"));
    handleEmail(`${form.getValues("email")}`, `${code}`);
  }

  function emailVerification() {
    const emailVerificationInput = document.getElementById(
      "verify_email"
    ) as HTMLInputElement;
    const emailVerification = emailVerificationInput?.value
      ? Number(emailVerificationInput.value)
      : null;

    if (emailVerification === emailCode) {
      console.log("approved", emailCode, emailVerification);
      // window.location.href = "/signin"
      return true;
    } else {
      console.log("emailCode", emailCode, "email", emailVerification);
      return false;
    }
  }

  const convertImageToString = async (imageUrl: string): Promise<string> => {
    if (
  ...
  async function onSignup() {
    //If input for phone, check for phone verification
    if (emailVerification()) {
      const userData = form.getValues();
      console.log("user added", userData);
      // const profile = convertImageToString(userData.profile);
          ...
          <div className="flex md:w-[50%] sm:w-full h-screen justify-center py-10 items-center">
            <Form {...form}>
              ...
              >
                        ...
                        <FormControl>
                          <div className="flex items-center border-2 py-3 rounded-lg mb-4 mt-1 border-gray-800 w-[80%] ml-[10%]">
                            ...
                            <input
                              placeholder="Email"
                              id="email"
                              className="pl-2 outline-none border-none ring-0 bg-gray-300 text-gray-800 w-[80%]"
                              {...field}
                            />
                          </div>
                        </FormControl>
                        ...
                        <FormControl>
                          <div className="flex items-center border-2 py-3 rounded-lg mb-3 mt-1 border-gray-800 w-[80%] ml-[10%]">
                            ...
                            <Image
                              ...
                              src={
                                isPasswordVisible ? "/eye.png" : "/eye_off.png"
                              }
                              alt="eye"
                              id="eye"
                              width={25}
                        ...
                        <FormControl>
                          <div className="flex items-center border-2 py-3 rounded-lg mb-3 mt-1 border-gray-800 w-[80%] ml-[10%]">
                            ...
                            <Image
                              className="mr-4 hover:cursor-pointer"
                              src={
                                isPasswordVisible2 ? "/eye.png" : "/eye_off.png"
                              }
                              alt="eye"
                              id="eye"
                              width={25}
                              height={25}
                              onClick={passwordEye2}
                            />
                            {/* <input placeholder="Password" id="password" className="pl-2 outline-none border-none ring-0 bg-gray-300 text-gray-800 w-[80%]" {...field} /> */}
                          </div>
                        </FormControl>
                        ...
                        <FormControl>
                          <div className="items-center mb-4">
                            ...
                            <div className="flex justify-center">
                              <button
                                className="bg-emerald-600 hover:bg-emerald-700 transition duration-300 px-3 py-2 rounded-lg text-gray-200 font-semibold cursor-pointer text-center text-lg"
                                onClick={phoneGeneration}
                              >
                                Send Code
                              </button>
                            </div>
                ...
                <p className="text-gray-800 text-[0.825rem] mb-2 mt-4 text-center px-[10%]">
                  By signing up, you accept our{" "}
                  <a
                    className="text-link hover:text-link_hover transition-colors duration-200 underline font-semibold"
                    href="/termsofservice"
                  >
