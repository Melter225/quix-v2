...
// Notes Accuracy
async function generateAccuracy(
  note: string
    ...
    messages: [
      {
        role: "system",
        content:
          'You are a helpful assistant in helping users learn more about their provided topics. If the user enters a prompt in a different language, ensure that you respond in English. If the user enters gibberish, a prompt that is incomprehensible, or a profane prompt, respond with just this message: No display. Do not include additional introductory and conclusive messages such as "Sure! Here is an accuracy measure for you:".',
      },
      {
        role: "user",
      ...
      if (accuracyValue != "No display" && accuracyValue != "No display.") {
        ...
        if (accuracy > 85) {
          ...
          await db
            ...
            .updateOne(
              { accuracy: note },
              { $set: { note, accuracy, topic, points, text } },
              { upsert: true }
            );
        }
